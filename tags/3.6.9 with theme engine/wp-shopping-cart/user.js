...
function change_variation(product_id) {
  //jQuery("*[id*='variation_select_"+product_id+"']").css('display','none');
  value_ids = '';
  form_id = "product_"+product_id;
...
jQuery(document).ready(function(){
	...
	jQuery('.wpsc_select_variation').change(
		function(){
      //jQuery("*[id*='variation_select_"+product_id+"']").css('display','none');
      product_id =jQuery('input[name=prodid]', jQuery(this).parents('form')).val();
      form_data = "ajax=true&get_updated_price=true&product_id="+product_id+"&"
		  form_data += jQuery('.wpsc_select_variation', jQuery(this).parents('form')).serialize();
		  //alert(form_data);
      ajax.post("index.php",return_price,form_data);
		}
	);
	
  jQuery('.additional_description_link').toggle(
		function(){		
      jQuery("span.additional_description",jQuery(this).parent()).css('display','inline');
      jQuery("img",this).attr('src',WPSC_URL+'/images/icon_window_collapse.gif');
      return false;
		},
		function(){		
      jQuery("span.additional_description",jQuery(this).parent()).css('display','none');
      jQuery("img",this).attr('src',WPSC_URL+'/images/icon_window_expand.gif');
      return false;
		}
