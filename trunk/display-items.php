...
if($_POST['submit_action'] == 'add')
  ...
  if(($_FILES['image'] != null) && preg_match("/\.(gif|jp(e)*g|png){1}$/",$_FILES['image']['name']))
    {
  ...
  if($_POST['file_url'] != null)
    ...
    if(is_file($filedir.$url_array[0]))
      {
  ...
  if(($_FILES['file']['name'] != null) || ($file_name != null))
    {
    $timestamp = time();
    $wpdb->query("INSERT INTO `".$wpdb->prefix."product_files` ( `filename`  , `mimetype` , `idhash` , `date` ) VALUES ( '', '', '', '$timestamp');");
    $fileid = $wpdb->get_var("SELECT `id` FROM `".$wpdb->prefix."product_files` WHERE `date` = '$timestamp'");
    ...
    if($_FILES['file']['name'] != null)
      ...
      //determing the mime type of files is a pain in the arse, it should be so much easier, it is not, hence, suxor
      if(function_exists('mime_content_type'))
        ...
        $mimetype = finfo_file($finfo_handle,$file_path);
        }
        else
        {
        if(preg_match("/(.mp3)+$/",$idhash))
          {
          $mimetype = 'audio/mpeg';
          }
          else
      ...
      if(function_exists("make_mp3_preview") && ($mimetype == 'audio/mpeg') && (!isset($_FILES['preview_file']['tmp_name'])))
        ...
        $wpdb->query("UPDATE `".$wpdb->prefix."product_files` SET `filename` = '".addslashes($file_name)."', `mimetype` = '$mimetype', `idhash` = '$idhash' WHERE `id` = '$fileid' LIMIT 1");
      }
    ...
    else if($_POST['select_product_file'] != null)
      {
      // handle select box for altering the downloadable file, need to return a sane name to previous file if it was uploaded using the form
      $fileid = $wpdb->get_var("SELECT `file` FROM `".$wpdb->prefix."product_list` WHERE `id` = '$id' LIMIT 1");
      
      if(file_exists(($filedir.$_POST['select_product_file'])))
        {        
        $file_path = $filedir.$_POST['select_product_file'];
        if(function_exists('finfo_open') && function_exists('finfo_file'))
          ...
          $mimetype = $mimetype_class->getType($file_path);
          } 
            
        $timestamp = time();
        $wpdb->query("INSERT INTO `".$wpdb->prefix."product_files` ( `filename`  , `mimetype` , `idhash` , `date` ) VALUES ( '".addslashes($_POST['select_product_file'])."', '$mimetype', '".addslashes($_POST['select_product_file'])."', '$timestamp');");                  
        $file = $wpdb->get_var("SELECT `id` FROM `".$wpdb->prefix."product_files` WHERE `date` = '$timestamp'");
        }
      }
      else
        {
...
if($_POST['submit_action'] == "edit")
  ...
  if(($_FILES['file']['name'] != null) || ($file_name != null))
    ...
    if($file_data != null)
      ...
      if($_FILES['file']['name'] != null)
        ...
        if(move_uploaded_file($_FILES['file']['tmp_name'],($filedir.$idhash)))
          ...
          $wpdb->query("UPDATE `".$wpdb->prefix."product_files` SET `filename` = '".addslashes($filename)."', `idhash` = '$idhash',  `mimetype` = '$mimetype' WHERE `id` = '".$file_data[0]['id']."' LIMIT 1");
          }
        }
        else
          {
          ...
          $wpdb->query("UPDATE `".$wpdb->prefix."product_files` SET `filename` = '".addslashes($file_name)."', `idhash` = '$idhash',  `mimetype` = '$mimetype' WHERE `id` = '".$file_data[0]['id']."' LIMIT 1");
          }
      }
      else
      {
      ...
      $wpdb->query("UPDATE `".$wpdb->prefix."product_files` SET `filename` = '".addslashes($filename)."', `mimetype` = '$mimetype', `idhash` = '$idhash' WHERE `id` = '$fileid' LIMIT 1");
      $wpdb->query("UPDATE `".$wpdb->prefix."product_list` SET `file` = '$fileid' WHERE `id` = '$id' LIMIT 1");
      if(file_exists($_FILES['preview_file']['tmp_name']) && ($_FILES['preview_file']['type'] == "audio/mpeg"))
        {
        copy($_FILES['preview_file']['tmp_name'], ($preview_clips_dir.$idhash.".mp3"));
        //make_mp3_preview(($filedir.$idhash), ($preview_clips_dir.$idhash.".mp3"));
      ...
      // handle select box for altering the downloadable file, need to return a sane name to previous file if it was uploaded using the form
      $fileid = $wpdb->get_var("SELECT `file` FROM `".$wpdb->prefix."product_list` WHERE `id` = '$id' LIMIT 1");
      $file_data = $wpdb->get_row("SELECT * FROM `".$wpdb->prefix."product_files` WHERE `id` = '$fileid' LIMIT 1",ARRAY_A);
      if($_POST['select_product_file'] == '.none.')
        {
        // unlikely that an7yone will ever upload a file called .none., so its the value used to signify clearing the product association
        $wpdb->query("UPDATE `".$wpdb->prefix."product_list` SET `file` = '0' WHERE `id` = '$id' LIMIT 1");
        }
        else if(file_exists($filedir.$_POST['select_product_file']))
        {
        $file_path = $filedir.$_POST['select_product_file'];
        if(function_exists('finfo_open') && function_exists('finfo_file'))
          {
        ...
        if($file_data != null)
          {
          if($file_data['filename'] != $file_data['idhash'])
            {
            if(!file_exists($filedir.$file_data['filename']))
              {
              rename($filedir.$file_data['idhash'], $filedir.$file_data['filename']);
              }
              else
              {
              //if there is already a file with the same name, prepend the current time to somewhat reduce the chance of overwrites
              rename($filedir.$file_data['idhash'], $filedir.time().$file_data['filename']);
              }
            }
          $wpdb->query("UPDATE `".$wpdb->prefix."product_files` SET `filename` = '".addslashes($_POST['select_product_file'])."', `mimetype` = '$mimetype', `idhash` = '".addslashes($_POST['select_product_file'])."' WHERE `id` = '$fileid' LIMIT 1");
          }
          else
          {        
          $fileid = $wpdb->get_var("SELECT `id` FROM `".$wpdb->prefix."product_files` WHERE `idhash` = '".addslashes($_POST['select_product_file'])."' LIMIT 1");
   ...
   if(file_exists($_FILES['preview_file']['tmp_name']) && ($_FILES['preview_file']['type'] == "audio/mpeg"))
     {
     $fileid = $wpdb->get_var("SELECT `file` FROM `".$wpdb->prefix."product_list` WHERE `id` = '$id' LIMIT 1");
     $file_data = $wpdb->get_row("SELECT `id`,`idhash` FROM `".$wpdb->prefix."product_files` WHERE `id` = '$fileid' LIMIT 1",ARRAY_A);
     $idhash = $file_data['idhash'];
     if(!is_dir($preview_clips_dir))
       {
       mkdir($preview_clips_dir);
       }
         ...
         if(is_numeric($key[0]) && is_numeric($key[1]) && (is_numeric($variation_data['stock']) || is_numeric($variation_data['price'])))
           ...
           if(is_numeric($variation_stock_data['id']))
             ...
             if($variation_sql != null)
               ...
               {
               $wpdb->query("INSERT INTO `".$wpdb->prefix."variation_priceandstock` ( `product_id` , `variation_id_1` , `variation_id_2` , `stock`, `price` ) VALUES ('".$_POST['prodid']."', '".$key[0]."', '".$key[1]."', '".$variation_stock."', '".$variation_price."');");
               }
           }
         }
       }
   ...
   if($_POST['donation'] == "yes")
    ...
    if(!($thumbnail_image == null && $_POST['image_resize'] == 3 && $_POST['current_thumbnail_image'] != null))
      ...
      if($thumbnail_image != null)
        ...
        {
        $wpdb->query("UPDATE `".$wpdb->prefix."product_list` SET `thumbnail_image` = '' WHERE `id`='".$_POST['prodid']."' LIMIT 1");
        }
      }
 ...
 */
  ...
  if(!is_writable($product_images))
    {
    $unwriteable_directories[] = $product_images;
    }
 
  if(!is_writable($imagedir))
    {
    $unwriteable_directories[] = $imagedir;
    }
  
  if(!is_writable($filedir))
    {
    $unwriteable_directories[] = $filedir;
    }
