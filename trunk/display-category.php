...
function display_category_row($category,$subcategory_level = 0)
  ...
  if($category['image'] !=null)
        {
        echo "<img src='../wp-content/plugins/wp-shopping-cart/category_images/".$category['image']."' title='".$category['name']."' alt='".$category['name']."' width='35' height='35' />";
        }
        else
          {
  ...
  if($_POST['submit_action'] == "add")
    ...
    $insertsql = "INSERT INTO `".$wpdb->prefix."product_categories` ( `name` , `description`, `image`, `fee` , `active`, `category_parent` ) VALUES ( '".$_POST['name']."', '".$_POST['description']."', '$image', '0', '1' ,'$parent_category')";
  ...
  if(($_POST['submit_action'] == "edit") && is_numeric($_POST['prodid']))
   ...
   if($_POST['notax'] == 'yes')
    ...
    if(is_numeric($_POST['category_parent']))
      {
      $parent_category = $_POST['category_parent'];
      $parent_category_sql = ", `category_parent` = '$parent_category' ";
      }
        
    $updatesql = "UPDATE `".$wpdb->prefix."product_categories` SET `name` = '".$wpdb->escape($_POST['title'])."', `description` = '".$wpdb->escape($_POST['description'])."' $parent_category_sql  WHERE `id`='".$_POST['prodid']."' LIMIT 1";
...
<div id='additem'>
  ...
  <table>
    ...
    <tr>
      ...
      <td>
        <?php echo wpsc_parent_category_list(0,0); ?>
      </td>
    </tr>
    <tr>
      <td>
