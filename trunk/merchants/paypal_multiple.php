...
function gateway_paypal_multiple($seperator, $sessionid)
  ...
  //if(get_option('paypal_use_ipn') == '1')
  ...
  foreach($cart as $item)
    ...
    $local_currency_shipping = nzshpcrt_determine_item_shipping($item['prodid'], $item['quantity'], $_SESSION['delivery_country']);
  ...
  if(($base_shipping >= 1) && ($all_donations == false))
    {
    $data['handling_cart'] = number_format($base_shipping,$decimal_places,'.','');
    }
  //$data['shipping'] = number_format($data['shipping'],$decimal_places,'.','');
  ...
  if($_POST['collected_data'][get_option('paypal_form_address')] != '')
    {   
    $data['address1'] = urlencode($_POST['collected_data'][get_option('paypal_form_address')]); 
//     $data['address2'] = "";
    }
...
//   if(preg_match("/^[a-zA-Z]{2}$/",$_POST['collected_data'][get_option('paypal_form_state')]))
//     {
//     $data['state'] = urlencode($_POST['collected_data'][get_option('paypal_form_state')]); 
//     }
  
//   if($_POST['collected_data'][get_option('paypal_form_post_code')] != '')
//     {   
