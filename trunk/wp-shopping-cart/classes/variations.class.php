...
class nzshpcrt_variations
  ...
  function variations_grid_view($product_id) 
    ...
    if(!((count($associated_variations) >= 1) && (count($associated_variations) <= 2)))
      {
      return "";
      exit();
      }
    ...
    if($variation_assoc_data != null)
      ...
      foreach($variation_assoc_data as $variation_association)
        ...
        if(($update_price === true) && (count($variation_ids) >= 1) && (count($variation_ids) <= 2))
          {
          $on_change = "onchange='change_variation($product_id, Array(\"".implode("\",\"", $variation_ids)."\"))'";
          } else { $on_change = ''; }
        $output .= "<select name='variation[".$variation_data['id']."]' $on_change >";
