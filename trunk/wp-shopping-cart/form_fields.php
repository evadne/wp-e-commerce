<?php
global $wpdb;
$form_types = Array("text","email","address","city","country","delivery_address","delivery_city","delivery_country","textarea","heading");
...
if($_POST['submit_action'] == 'add')
  ...
  if($_POST['new_form_name'] != null)
    ...
    foreach($_POST['new_form_name'] as $form_id => $form_name) 
      ...
      $wpdb->query("INSERT INTO `".$wpdb->prefix."collect_data_forms` VALUES ('', '$form_name', '$form_type', '$form_mandatory', '$form_display_log', '', '1','".$order_number."');");
      }
    }
...
<div class="wrap">
  ...
  foreach($form_data as $form_field)
    ...
    echo "      <td style='text-align: center; width: 12px;'><a class='image_link' href='#' onclick='return remove_form_field(\"form_id_".$form_field['id']."\",".$form_field['id'].");'><img src='../wp-content/plugins/wp-shopping-cart/images/trash.gif' alt='".TXT_WPSC_DELETE."' title='".TXT_WPSC_DELETE."' /></a>";
    echo "</td>";
  ...
  <table>
    <tr>
      ...
      <label for='payment_method_1'><?php echo $gateway_name."/".TXT_WPSC_CREDIT_CARD . TXT_WPSC_DEFAULT_GATEWAY_ONLY; ?></label>
      </td>
    </tr>
    <tr>
      <td colspan='2'>
