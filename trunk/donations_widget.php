...
function widget_donations($args)
  ...
  $donation_count = $wpdb->get_var("SELECT COUNT(*) AS `count` FROM `".$wpdb->prefix."product_list` WHERE `donation` = '1'");   
  if($donation_count > 0)
    {
...
function widget_donations_init()
 ...
 function nzshpcrt_donations($input = null)
   ...
   $sql = "SELECT * FROM `".$wpdb->prefix."product_list` WHERE `donation` = '1'";
   ...
   if($products != null)
     ...
     foreach($products as $product)
       ...
       if($product['image'] != null)
         {
        $output .= "<img src='$siteurl/wp-content/plugins/wp-shopping-cart/product_images/thumbnails/".$product['image']."' title='".$product['name']."' alt='".$product['name']."' /><br />";
        }
