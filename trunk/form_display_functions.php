<?php
function categorylist($product_id = '', $unique_id = '', $category_id = null, $iteration = 0)
  {
  /*
  ...
  foreach((array)$values as $option)
    ...
    $output .= categorylist($product_id, $unique_id, $option['id'], $iteration+1);
    $selected = "";
    }
  return $output;
  }
...
function wpsc_parent_category_list($category_id, $category_parent_id)
  {
  global $wpdb,$category_data;
  $options = "";
  $values = $wpdb->get_results("SELECT * FROM `".$wpdb->prefix."product_categories` WHERE `active` = '1' AND `id` != '$category_id' ORDER BY `id` ASC",ARRAY_A);
  $options .= "<option value='0'>".TXT_WPSC_SELECT_PARENT."</option>\r\n";
  $options .= wpsc_category_options($category_id, null, 0, $category_parent_id);   
  $concat .= "<select name='category_parent'>".$options."</select>\r\n";    
  return $concat;
  }

function wpsc_category_options($this_category = null, $category_id = null, $iteration = 0, $selected_id = null)
  {
  /*
   * Displays the category forms for adding and editing products
   * Recurses to generate the branched view for subcategories
  ...
  if(is_numeric($category_id))
    {
    $values = $wpdb->get_results("SELECT * FROM `".$wpdb->prefix."product_categories` WHERE `active`='1' AND `id` != '$this_category' AND `category_parent` = '$category_id'  ORDER BY `id` ASC",ARRAY_A);
    }
    else
    {
    $values = $wpdb->get_results("SELECT * FROM `".$wpdb->prefix."product_categories` WHERE `active`='1' AND `id` != '$this_category' AND `category_parent` = '0'  ORDER BY `id` ASC",ARRAY_A);
    }
  foreach((array)$values as $option)
    {
    ...
    $output .= wpsc_category_options($this_category, $option['id'], $iteration+1, $selected_id);
    $selected = "";
    }
  return $output;
  }
...
function wpsc_uploaded_files()
  ...
  while(($file = @readdir($dir)) !== false)
    ...
    if(($file != "..") && ($file != ".")&& ($file != "product_files")  && ($file != ".DS_Store") && !stristr($file, "~") && !( strpos($file, ".") === 0 ))
      {
