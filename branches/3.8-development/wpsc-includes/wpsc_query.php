...
//		exit('<pre>'.print_r($wp_query, true).'</pre>');
	...
	/*
		...
		if($this->query_vars['product_url_name'] != null) {
			$names_to_attempt = array();
			$names_to_attempt[] = stripslashes($this->query_vars['product_url_name']);
			$names_to_attempt[] = sanitize_title(stripslashes($this->query_vars['product_url_name']));
		
			$product_id = $wpdb->get_var("SELECT `product_id` FROM `".WPSC_TABLE_PRODUCTMETA."` WHERE `meta_key` IN ( 'url_name' ) AND `meta_value` IN ( '".implode("', '",$names_to_attempt )."' ) ORDER BY `product_id` DESC LIMIT 1");
		} else {
		...
		if(function_exists('gold_shpcrt_search_sql') && ($_GET['product_search'] != '')) {
			...
			$term_id = $wpdb->get_var("SELECT `term_id` FROM `{$wpdb->terms}` WHERE slug='$tag' LIMIT 1");
			
			
			$taxonomy_id = $wpdb->get_var("SELECT `term_taxonomy_id` FROM `{$wpdb->term_taxonomy}` WHERE term_id = '".$term_id."' AND taxonomy='product_tag' LIMIT 1");
			
			$product_ids = $wpdb->get_col("SELECT `object_id` FROM `{$wpdb->term_relationships}` WHERE term_taxonomy_id = '".$taxonomy_id."'");
		
			$sql = "SELECT * FROM ".WPSC_TABLE_PRODUCT_LIST." WHERE id IN (".implode(",",$product_ids).") AND `publish` IN('1') AND `active` IN('1') LIMIT ".count($product_ids).""; //Transom - added publish & active
		} else {
			...
			/*	$rowcount = $wpdb->get_var("SELECT COUNT( DISTINCT `products`.`id`) AS `count`
				 ...
				 AND `cat_assoc`.`category_id` IN ('".implode("','", $this->category_id_list)."')
				 $no_donations_sql");
				...
				$sql = "SELECT DISTINCT `products`.* , `cat_assoc`.`category_id` , `order`.`order` , IF( ISNULL( `order`.`order` ) , 0, 1 ) AS `order_state`
				FROM `".WPSC_TABLE_PRODUCT_LIST."` AS `products`
				LEFT JOIN `".WPSC_TABLE_ITEM_CATEGORY_ASSOC."` AS `cat_assoc`
					ON `products`.`id` = `cat_assoc`.`product_id`
				LEFT JOIN `".WPSC_TABLE_PRODUCT_ORDER."` AS `order`
				...
				WHERE `products`.`publish`='1'
				AND `products`.`active`='1'
				$no_donations_sql $group_sql
				ORDER BY `products`.`special`, $order_by
