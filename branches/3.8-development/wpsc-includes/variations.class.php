  ...
  function variations_grid_view($product_id, $variation_ids = null,  $variation_values = null, $selected_price = null, $limited_stock = null) {
    ...
    $priceandstock_id_string = implode(", ", $priceandstock_ids);
		if(count($priceandstock_ids)) {
			$unindexed_variation_properties_data = $wpdb->get_results("SELECT `id`, `stock`, `price`, `special_price`, `sku`, `weight`,`weight_unit`, `file` FROM `".WPSC_TABLE_VARIATION_PROPERTIES."` WHERE `id` IN({$priceandstock_id_string})", ARRAY_A);
		}
    ...
    // add variation code ends
			foreach((array)$associated_variation_values as $key => $variation_set) {
				...
				if(isset($variation_properties_data[$variation_set['priceandstock_id']])) {
					...
					$product_special_price = $variation_properties_data[$variation_set['priceandstock_id']]['special_price'];
					...
					$product_special_price = $product_data['special_price'];
				...
				$output .= "    <td class='sku' ><input type='text' name='variation_priceandstock[{$variation_ids}][sku]' value='$product_sku' size='6' /></td>\n\r";
				...
				$output .= "    <td class='price'><input type='text' name='variation_priceandstock[{$variation_ids}][special_price]' value='$product_special_price' size='6' /></td>\n\r";
  ...
  */
  function update_variation_values($product_id, $variation_values) {
    ...
    //exit("<pre>".print_r($_POST,true)."</pre>");
      ...
      $variation_special_price = (float)str_replace(",","",$variation_data['special_price']);
      ...
      if((is_numeric($variation_data['stock']) || is_numeric($variation_price))) {
        ...
        if(is_numeric($variation_stock_data['id'])) {
          ...
          if(($variation_stock_data['special_price'] != $variation_special_price)) {
            $variation_sql[] = "`special_price` = '{$variation_special_price}'";
          }
          ...
          if(($variation_stock_data['file'] != $variation_file)) {
            $variation_sql[] = "`file` = '{$variation_file}'";
					...
					//exit("<pre>".print_r($variation_sql,true)."</pre>");
