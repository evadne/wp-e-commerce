...
function widget_specials($args) {
  ...
  $special_count = $wpdb->get_var("SELECT COUNT(*) AS `count` FROM `".WPSC_TABLE_PRODUCT_LIST."` WHERE `special` = '1'  AND `active` IN ('1') AND `publish` IN ('1')");   
  if($special_count > 0) {
 ...
 function nzshpcrt_specials($input = null) {
   ...
   $sql = "SELECT * FROM `".WPSC_TABLE_PRODUCT_LIST."` WHERE `special` = '1'  AND `active` IN ('1') AND `publish` IN ('1') ORDER BY RAND() LIMIT 1";
